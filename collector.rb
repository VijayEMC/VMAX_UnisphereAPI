...
#settings_file=("~/grafanaApp/rubyApp/app/settings.json")
#########################
...
influxdb = InfluxDB::Client.new config['influx']['table'], host: config['influx']['ip'], port: config['influx']['port'], username: config['influx']['user'], password: config['influx']['pass']
...
unisphereArray = config['unisphere']
###########################################
# Unisphere Instance Loop 
# Iterate through each Unisphere Instance
###########################################
unisphereArray.each do |uniInst|
        
    # Build our url strings
    keys_url = "https://#{uniInst['ip']}:#{uniInst['port']}/univmax/restapi/performance/Array/keys"
    metrics_url = "https://#{uniInst['ip']}:#{uniInst['port']}/univmax/restapi/performance/Array/metrics" 

    # Create base 64 encoded auth
    auth = Base64.strict_encode64("#{uniInst['user']}:#{uniInst['password']}")
